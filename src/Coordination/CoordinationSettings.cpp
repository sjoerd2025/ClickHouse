...
#define LIST_OF_COORDINATION_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(UInt64, write_snapshot_version, 8, "Snapshot format version to write (supported: 8 and above). Increase only after all nodes in the cluster are upgraded to a version that supports the new format", 0) \
...
void KeeperConfigurationAndSettings::dump(WriteBufferFromOwnString & buf) const
{
    ...
    writeText("ttl_gc_period_ms=", buf);
    write_int(static_cast<uint64_t>(coordination_settings[CoordinationSetting::ttl_gc_period_ms]));
